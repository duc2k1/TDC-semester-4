...
{
    // Lay so like theo product id
    public function loadLike($id)
    {
        //2. Viết câu SQL
        $sql = parent::$connection->prepare("SELECT * FROM products WHERE id=?");
        $sql->bind_param('i', $id);
        return parent::select($sql)[0]['product_like'];
    }
    // Giam like theo product id
    public function removeLike($id)
    {
        //2. Viết câu SQL
        $sql = parent::$connection->prepare("SELECT * FROM products WHERE id=?");
        $sql->bind_param('i', $id);
        var_dump(parent::select($sql)[0]['product_like']);
        $like = parent::select($sql)[0]['product_like'] - 1;
        $sql = parent::$connection->prepare("UPDATE products SET product_like=? WHERE id=?");
        $sql->bind_param('ii', $like, $id);
        return $sql->execute();
    }
    // Thêm like theo product id
